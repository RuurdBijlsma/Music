...
export const useSpotifyApiStore = defineStore('spotify-api', () => {
  ...
  const getArtist = async (id: string, useCache = false) => await get(api.getArtist, [id], useCache)
  const getArtistAlbums = async (id: string) => await get(api.getArtistAlbums, [id])
  const getArtistRelatedArtists = async (id: string) => await get(api.getArtistRelatedArtists, [id])
