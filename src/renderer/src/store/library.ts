...
export const useLibraryStore = defineStore('library', () => {
  ...
  async function loadLikedTracks() {
    ...
    for (const localTrack of tracksRef.value) {
      ...
      if (localTrack.id.startsWith('yt-') || !apiTracks.hasOwnProperty(localTrack.id)) continue
  ...
  async function addToPlaylist(playlistId: string, track: SpotifyApi.TrackObjectFull) {
    ...
    const trackInPlaylist = viewedPlaylist.value.tracks.items.find((t) => t.track.id === track.id)
  ...
  async function removeFromPlaylist(playlistId: string, trackUri: string) {
    ...
    if (viewedPlaylist.value === null) return
    const trackIndex = viewedPlaylist.value.tracks.items.findIndex((t) => t.track.uri === trackUri)
