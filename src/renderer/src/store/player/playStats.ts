...
export const useStatsStore = defineStore('playerStats', () => {
  ...
  async function collectMinuteStats(collection: ItemCollection, track: SpotifyApi.TrackObjectFull) {
    // gets called after every 60 seconds of audio play is elapsed
    const db = await baseDb
  ...
  async function getTopFromIndex<T>(store: string, indexKey: string, limit = 10, highToLow = true) {
    const db = await baseDb
  ...
  async function generateWrapStats(trackLimit = 10, artistLimit = 5) {
    ...
    const topTracks = await getTopFromIndex<TrackStat>('trackStats', 'listenMinutes', trackLimit)
    // MOST SKIPPED STUFF
