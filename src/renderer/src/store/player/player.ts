...
export const usePlayerStore = defineStore('player', () => {
  ...
  function playTrackData(_collection: ItemCollection, trackData: TrackData) {
    ...
    if (
      ...
      if (trackData.likedInfo?.startTime) playerElement.currentTime = trackData.likedInfo?.startTime
    }
  }
    ...
    if (!isTrackDataLoading.has(_trackId)) {
      ...
      trackLoader
        .getTrackData(_track, (trackData) => {
          if (trackLoader.isLoadedTrackData(trackData)) isTrackDataLoading.delete(_trackId)
  ...
  function renderProgress() {
    ...
    for (let i = 0; i < canvasBars.binPos.length; i++) {
      ...
      if (x / canvas.width < currentTime.value / duration.value) context.fillStyle = ui.themeColor
  ...
  async function skip(n = 1) {
    ...
    if (n > 0 && currentTime.value / duration.value < 0.9) {
      if (track.value !== null) playerStats.collectSkipStat(collection.value, track.value).then()
    }
