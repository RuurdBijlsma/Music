  ...
  <div class="wrapped">
    ...
    <v-list class="stat-list">
      ...
      <template v-for="({ artist, listenMinutes, history }, i) of artistsTop">
        <v-list-item rounded @click.right="dialog.setContextMenuItem($event as MouseEvent, artist)">
          <v-list-item-title class="item-stat">
            <p class="item-rank">{{ i + 1 }}</p>
            <v-avatar :image="itemImage(artist as SpotifyApi.ArtistObjectFull)" size="36px" />
            <router-link :to="itemUrl(artist)" class="flex-grow-1" no-style
              >{{ artist.name }}
            </router-link>
            <p class="minutes-stat align-right">
              <span class="font-weight-bold">{{ listenMinutes.toLocaleString() }}</span>
              mins
            </p>
            <v-btn
              density="compact"
