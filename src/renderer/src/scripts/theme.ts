...
function timeUntilSwitch(lightTime: string, darkTime: string) {
  ...
  let msUntilDark = (darkHours - nowHours) * 60 * 60 * 1000 + (darkMinutes - nowMinutes) * 60 * 1000
  let msUntilLight =
...
export function getThemeFromLocalStorage() {
  ...
  switch (localStorage.theme) {
    ...
    case 'light':
      ...
      break
    case 'system':
      themeString = window.matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light'
      break
    case 'schedule':
