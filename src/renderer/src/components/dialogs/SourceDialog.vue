<template>
  <v-dialog v-model="source.show" :class="{ dark: ui.isDark }" :scrollable="true" width="auto">
    <div class="translucent">
      ...
      >
        ...
        <template v-else>
          <div v-for="(item, i) in source.items" class="yt-card mb-5">
            <v-img :src="getImg(item)" class="image" height="200" max-width="355" min-width="355">
              <p class="duration">
            ...
            <div class="info-content">
              ...
              <div class="channel mt-2">
                <v-avatar :color="randomColor(item.id)">{{ item.channelTitle[0] }} </v-avatar>
                <span class="ml-4">{{ item.channelTitle }}</span>
              </div>
              <spacer></spacer>
              <div class="actions mt-4">
                <v-btn v-if="youtubeSource !== item.id" variant="tonal" @click="activate(item)">
                  activate
                </v-btn>
                <simple-yt-player
      ...
      <v-divider></v-divider>
      <v-card-actions>
        <v-btn :block="true" :color="ui.themeColor" @click="source.show = false">Dismiss </v-btn>
      </v-card-actions>
    </div>
  </v-dialog>
</template>
