<template>
  <div class="left-nav">
    <v-btn
      ...
      variant="text"
    ></v-btn>
    <span :class="{ active: route.path === '/' }" class="button-text nav-1-text">Listen Now</span>
    <v-btn
      ...
      variant="text"
    ></v-btn>
    <span :class="{ active: route.path === '/browse' }" class="button-text nav-2-text">Browse</span>
    <v-btn
    ...
    >
      <v-tooltip v-for="playlist in library.saved.playlist" :text="playlist.name" location="right">
