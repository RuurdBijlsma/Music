...
export default class NodeFunctions {
    ...
    if (!(await this.checkFileExists(this.ffmpegPath))) {
      if (os.platform() !== 'win32') {
        child_process.exec('ffmpeg', (_, stdout, stderr) => {
          ...
          if (!stdout.startsWith('ffmpeg version') && !stderr.startsWith('ffmpeg version')) {
  ...
  async getDominantColor(imageFile: string) {
    ...
    const pickColor = (bgColor: number[]) => {
      const contrasts = rgbs.map(([r, g, b]: number[]) => getContrastRatio([r, g, b], bgColor))
  ...
  configUpdater() {
    ...
    autoUpdater.on('update-available', (e) => this.webLog('[auto updater]', 'update-available', e))
    ...
    return new Promise<any[]>(async (resolve) => {
      ...
      if (process.platform === 'win32') {
        const command =
          ...
          [`${this.ytdlpPath}`, ...args]
            .map((a) => (a.includes(' ') && !a.includes('"') ? `"${a.replaceAll('"', '\\"')}"` : a))
            .join(' ')
