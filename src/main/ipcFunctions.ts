...
export function handleIpc(ipcMain: Electron.IpcMain, win: BrowserWindow) {
    ...
    ipcMain.handle("searchYt",
        (_, query: string, results: number = 3) => nf.searchYouTube(query, results));
    ipcMain.handle("ytInfoById",
        (_, id: string) => nf.youTubeInfoById(id));
    ipcMain.handle("getDominantColor",
        (_, imgUrl: string) => nf.getDominantColor(imgUrl));
    ipcMain.handle("setPlatformPlaying"
        , (_, value: boolean, darkTheme: boolean) => nf.setPlatformPlaying(value, darkTheme));
    ipcMain.handle("stopPlatformPlaying",
        (_) => nf.stopPlatformPlaying());
    ipcMain.handle("getOutputDirectory",
        (_) => nf.getOutputDirectory());
    ipcMain.handle("downloadAsJpg",
        (_, imgUrl: string) => nf.downloadAsJpg(imgUrl));
    ipcMain.handle("getVolumeStats",
        (_, trackFile: string) => nf.getVolumeStats(trackFile));
    ipcMain.handle("getDirectories",
        () => Directories);
    ...
    ipcMain.handle("firstLogin", (_, spotifyAuth: {
